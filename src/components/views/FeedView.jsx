...
const FeedView = ({ user, onViewProfile }) => {
  ...
  // ✅ Conteo REAL desde post_comments (no depende de posts.comments_count)
  ...
  const fetchPosts = async () => {
    ...
    if (votesError) console.warn('Error cargando votos:', votesError);

    const voteMap = {};
    if (userVotes) userVotes.forEach(v => (voteMap[v.post_id] = v.vote_type));

    // ✅ Traer conteo real de comentarios por post (robusto aunque no haya relaciones FK)
