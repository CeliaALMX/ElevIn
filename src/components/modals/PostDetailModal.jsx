...
const PostDetailModal = ({ post, onClose, user, commentsData, commentActions, fetchComments }) => {
  ...
  // Variable clave para nuestro diseño adaptativo
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    await commentActions.add(post.id, newComment, post.user_id); // Aseguramos que envíe la notificación
    setNewComment('');
    setIsSending(false);
  };
  ...
  const currentMedia = hasMedia ? mediaList[currentMediaIndex] : null;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-0 md:p-4 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      ...
      <div 
        // Si no hay media, limitamos el ancho para que parezca un modal elegante, si hay, lo hacemos gigante
        className={`relative w-full bg-white dark:bg-slate-900 md:rounded-xl overflow-hidden shadow-2xl flex flex-col md:flex-row h-[100dvh] md:h-[90vh] max-h-screen ${hasMedia ? 'max-w-7xl' : 'max-w-2xl'}`} 
        onClick={e => e.stopPropagation()}
      >
        
        {/* LADO IZQUIERDO: MEDIA (CARRUSEL) - SOLO SE RENDERIZA SI HAY MEDIA */}
        {hasMedia && (
        ...
        {/* Si no hay media, w-full ocupa el 100%. Si hay media, limitamos a 400px en escritorio */}
        <div className={`flex flex-col h-full w-full ${hasMedia ? 'md:w-[400px] border-l dark:border-slate-700' : 'md:w-full'} bg-white dark:bg-slate-900 overflow-hidden`}>
          ...
          <div className="p-4 border-b dark:border-slate-700 bg-white dark:bg-slate-900 shrink-0 z-10">
             ...
             {post.content && (
               // Si no hay media, le damos más espacio al texto (max-h-60), si hay media, lo mantenemos en 24
               <div className={`mt-3 text-sm text-gray-800 dark:text-gray-200 overflow-y-auto whitespace-pre-wrap scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 ${hasMedia ? 'max-h-24' : 'max-h-60 text-base leading-relaxed'}`}>
                 {post.content}
               </div>
             )}
          </div>
             ...
             <div className="flex gap-3 items-center">
                ...
                <div className="flex-1 relative">
                  <input 
                    ...
                    disabled={isSending}
                    className="w-full text-sm py-2.5 pl-4 pr-12 rounded-full bg-gray-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-blue-500 dark:text-white transition-all shadow-sm"
